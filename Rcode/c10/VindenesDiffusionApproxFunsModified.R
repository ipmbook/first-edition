...
V <- function (y, envar, demvar, ensd, bconst) {
  return ((exp((ensd*y + bconst)/2) -
           demvar/envar*exp(-(ensd*y + bconst)/2))^2 / 4)
}
...
###################################################################################
# Code contributed by Robin Snyder, to implement the Lande (1998; Oikos 83: 353-358)  
# diffusion approximation on a scale with isotropic noise. 
################################################################################

mu.nu.Lande <- function(y, envar, demvar, lambda, ensd, bconst){
    mu <- (log(lambda) - envar/2 -
           demvar/(4*V(y, envar, demvar, ensd, bconst))) /
             sqrt(envar + demvar/V(y, envar, demvar, ensd, bconst))
    nu <- rep (1, length(y))
    mu[y<=0] <- 0
    nu[y<=0] <- 0
    return(list("mu"=mu, "nu"=nu))
...
diffusion.Lande <- function(y0, t.max=1000, delta.t=.1, n.sim=1000, b=Inf,
  ...
  j <- 1
  for (i in 1:(t.max/delta.t)) {
    mn  <- mu.nu.Lande(y=y, envar=envar, demvar=demvar, lambda=lambda,
                       ensd=ensd, bconst=bconst)
    y <- y + mn$mu*delta.t + rnorm(n.sim)*sqrt(mn$nu*delta.t)
    y[y>=b] <- b
    y[y<=0] <- 0
    if (i%%(1/delta.t)==0) {
