...
pmiss1 <- 1/(1+exp(-(-8 * (cohortData$z-mean(cohortData$z)))))
ii <- sample(nrow(cohortData), 6000, replace=FALSE, prob=pmiss1)
cohortData$z[ii] <- NA
imputeData <- prune_data(cohortData)
...
pmiss2 <- 1/(1+exp(-(+8 * (cohortData$z-mean(cohortData$z)))))
ii <- sample(nrow(cohortData), 2600, replace=FALSE, prob=pmiss2)
cohortData$z[ii] <- NA
imputeData <- prune_data(cohortData)
...
plot(zimpt~zorig, data=allDataMCAR, xlab="True Size", ylab="Imputed Size",
     xlim=c(2.5,3.5),ylim=c(2.5,3.5),cex=0.75)
abline(0, 1, lty=3); add_panel_label("b"); 

plot(with(allDataMCAR, density(na.exclude(zimpt-zorig), bw=0.05)),
     col="black", main="",xlab="Imputed size - Actual size",
     ylab="Probability density"); 
     abline(v=0,lty=3); add_panel_label("c"); 

plot(with(allDataNMAR1, density(na.exclude(zimpt-zorig), bw=0.05)),
      main="",xlab="Imputed size - Actual size",
      ylab="Probability density"); 
lines(with(allDataNMAR2,  density(na.exclude(zimpt-zorig), bw=0.05)),
      lty=2,lwd=2);  
      abline(v=0,lty=3); add_panel_label("d"); 

dev.copy2eps(file="./c10/figures/UngulateImputation.eps"); 
