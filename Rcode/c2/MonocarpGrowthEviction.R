...
## Assumes R's working directory is set to the location
## of this file, and MonocarpTrueIPMfunctions is in the
## same location
...
# par(cex.axis=1.2,cex.lab=1.2,yaxs="i",xaxs="i",bty="l",
    # mfrow=c(2,2),mgp=c(2.5,1,0),mar=c(4,5,2,2));
    
set_graph_pars("panel4");

zvals <- seq(-2.65,4,length=200); 
tiny <- .001
g1 <- G_z1z(zvals,min(zvals),m.par.true); g1[g1<tiny] <- NA;
g2 <- G_z1z(zvals,mean(zvals),m.par.true); g2[g2<tiny] <- NA;
g3 <- G_z1z(zvals,max(zvals),m.par.true);g3[g3<tiny] <- NA;

matplot(zvals,cbind(g1,g2,g3),type="l",lty=1,lwd=c(1,2,3),col="black",
xlab="Final size z'",ylab="Probabity density",ylim=c(0,1));
...
plot(zvals,Wvals,type="l",lty=1,lwd=2,col="black",
xlab="Initial size z",ylab="Fraction wrongfully evicted",ylim=c(0,1.1*max(Wvals)));
...
par(xaxs="r")
zvals=seq(3,4.5,length=25); Nvals=b_z(zvals,m.par.true); 
...
#dev.copy2eps(file="~/Repos/ipm_book/c2/figures/MonocarpGrowthEviction.eps");
