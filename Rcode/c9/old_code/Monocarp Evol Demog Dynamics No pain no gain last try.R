...
########### Plot mean and variance of flowering intercept over time, 10 reps. 
...
iterate_model <- function(params,n.iter,init.beta.mean,init.beta.sd) {
	...
	var.beta2[1] <- sum(flow.int.s*beta2*beta2)/sum(flow.int.s) - mean.beta2[1]*mean.beta2[1]
	...
	var.beta[gen] <- sum(apply(nt1,2,sum)*beta2/sum(nt1)) - 	  mean.beta[gen]*mean.beta[gen]
	
	mean.beta2[gen] <- sum(apply(nt1,2,sum)*(beta2)/sum(nt1))
	var.beta2[gen] <- sum(apply(nt1,2,sum)*beta2*beta2/sum(nt1)) - 	  mean.beta2[gen]*mean.beta2[gen]
...
U.beta <- max(max.beta[1:5]) + 2 
L.beta <-  min(min.beta[1:5]) -2

h.beta <- (U.beta - L.beta)/nBigMatrix.beta 
meshpts.beta <- L.beta + ((1:nBigMatrix.beta) - 1/2) * h.beta

U.z <- max(max.z[1:5]) * 1.2 
L.z <- min(min.z[1:5]) * 1.2 
...
U.beta <- max(max.beta[6:10]) + 2 
L.beta <-  min(min.beta[6:10]) -2

h.beta <- (U.beta - L.beta)/nBigMatrix.beta 
meshpts.beta <- L.beta + ((1:nBigMatrix.beta) - 1/2) * h.beta

U.z <- max(max.z[6:10]) * 1.2 
L.z <- min(min.z[6:10]) * 1.2 
...
plot(1:n.yrs,sol.2D.minus20$mean.beta2,col="green",type="l",ylim=c(0,1000))
points(1:n.yrs,sol.2D.minus30$mean.beta2,col="blue",type="l")
add_panel_label("c")
...
plot(1:n.yrs,sol.2D.minus20$var.beta2,col="green",type="l",ylim=c(0,10000))
points(1:n.yrs,minus20.var,col="red",type="l")
points(1:n.yrs,sol.2D.minus30$var.beta2,col="blue",type="l")
points(1:n.yrs,minus30.var,col="red",type="l")
add_panel_label("d")

#the Gaussian assumption looks pretty good - after 1st step!
...
#Mean S at time t
	...
	cat(gen,"  ",beta.t.mean[gen-1],"  ",S.t.mean[gen-1],"\n")
...
points(1:n.yrs,sol.2D.minus20$mean.beta,col="green",type="l")
points(1:n.yrs,sol.2D.minus30$mean.beta,col="blue",type="l")
points(1:n.yrs,beta.minus20$beta.t.mean,col="red",type="l")
points(1:n.yrs,beta.minus30$beta.t.mean,col="red",type="l")

add_panel_label("a")

#Approx var

minus20.var.beta <- beta.minus20$S.t.mean - beta.minus20$beta.t.mean * beta.minus20$beta.t.mean

minus30.var.beta <- beta.minus30$S.t.mean - beta.minus30$beta.t.mean * beta.minus30$beta.t.mean
...
points(1:n.yrs,minus20.var.beta,col="red",type="l")
points(1:n.yrs,minus30.var.beta,col="red",type="l")

add_panel_label("b")

#so the approximation starting at -30 is bang on the other is not  
#the -20 one converges to the right answer wrong sign!

plot(1:n.yrs,sol.2D.minus20$mean.beta2,col="green",type="l",ylim=c(0,1000))
points(1:n.yrs,beta.minus20$S.t.mean,col="red",type="l")

points(1:n.yrs,sol.2D.minus30$mean.beta2,col="blue",type="l")
points(1:n.yrs,beta.minus30$S.t.mean,col="red",type="l")
add_panel_label("c")
...
#the assumption of a Gaussian distribution of beta seeds good
