...
flow.ints <- matrix(NA,nrow=n.yrs,ncol=10)
for(reps in 1:10){
	init.mean.flow.int <- ifelse(reps<6, -20, -30)
	source("Monocarp Simulate Evol IBM.R") 
    flow.ints[,reps] <- mean.flow.int
    cat(reps,"   ",init.mean.flow.int,"\n")
}


set_graph_pars("panel1"); 
matplot(flow.ints,type="l",col=rep(c("black","grey"),rep(5,2)),xlab="Time",ylab="Mean flowering intercept")
...
max.z.minus20 <- max.z.t
min.z.minus20 <- min.z.t
...
# 
# max.z.minus30 <- max.z.t
# min.z.minus30 <- min.z.t
...
abline(h=-24.33668,col="red") #adding ESS from calculation below
...
flow.int.s <- dnorm(meshpts.beta, mean = init.mean.flow.int, sd = init.beta.sd)
...
delta <- init.mean.flow.int/100
G.beta <- init.beta.sd*init.beta.sd
...
for(gen in 2:n.iter){
	...
	params["p.r"] <- 1
	equ.p.r <- 1/R0_calc(params["flow.int"])
	params["p.r"] <- equ.p.r
	...
	d2.lambda.d2.beta <- (lambda.up - 2 + lambda.down)/(delta*delta)
	
	beta.t.mean[gen] <- beta.t.mean[gen-1] + d.lambda.d.beta * beta.t.var[gen-1]
	beta.t.var[gen] <- (beta.t.var[gen-1]  + d2.lambda.d2.beta * (beta.t.var[gen-1]^2))*
	(1+beta.off.sd*beta.off.sd)
	
	params["flow.int"] <- beta.t.mean[gen]
	
	cat(gen,"  ",beta.t.mean[gen],"  ",beta.t.var[gen],"\n")
	cat(gen,"  ",d.lambda.d.beta,"  ",d2.lambda.d2.beta,"\n")
