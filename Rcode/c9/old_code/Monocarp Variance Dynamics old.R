...
save(mean.flow.ints,var.flow.ints,min.z,max.z,min.beta,max.beta,file="SimData2.Rdata")
}
...
plot(1:n.yrs,sol.2D.minus20$var.beta2,col="blue",type="l",ylim=c(0,10000),lwd=4,ylab="Variance of beta^2")
...
#####################################################################################
# THIRD PLOT: Approximate dynamics using Iwasa et al. 1991 Evolution
#####################################################################################
nBigMatrix <- nBigMatrix.z; 

### params,init.mean.flow.int,n.iter,delta,var.beta.t,prop.Recr
beta.minus20 <- Approx_dynamics2(m.par.true,-20,n.yrs,0.25,sol.2D.minus20$var.beta,sol.2D.minus20$prop.Recr)
beta.minus30 <- Approx_dynamics2(m.par.true,-30,n.yrs,0.25,sol.2D.minus30$var.beta,sol.2D.minus30$prop.Recr)
...
minus20.var.beta <- beta.minus20$S.t.mean - beta.minus20$beta.t.mean^2
minus30.var.beta <- beta.minus30$S.t.mean - beta.minus30$beta.t.mean^2

matplot(var.flow.ints,type="l",col=rep(c("black","grey"),rep(5,2)),xlab="Time",
	ylab="Variance beta",xlim=c(1,n.yrs),ylim=c(-6,6))
...
points(1:n.yrs,minus20.var.beta,col="purple",type="l",lwd=2,lty=2)
points(1:n.yrs,minus30.var.beta,col="red",type="l",lwd=2,lty=2)
add_panel_label("b")
...
plot(1:n.yrs,sol.2D.minus20$var.beta2,col="blue",type="l",ylim=c(0,10000),ylab="Variance of beta^2",
	lty=1,lwd=4)
