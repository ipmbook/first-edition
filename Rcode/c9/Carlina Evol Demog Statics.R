...
#wrapper function, changes the flowering strategy then calculated the invader's stochastic growth rate
#note the arguments are the yearly parameters and the change in flowering strategy,
#hence we add beta.0 onto the resident's flowering strategy.
...
points(flow.inter,Ls,type="b",pch=19)

points(-14.34,0,pch=19,col="red")
...
# # for(i in 1:n.betas){
	...
	# #set resident flowering strategy
	
	# params.res["flow.int"] <- beta.s[i]
	
	# #make resident environment
	
	# iter.R<-iterate_model(params.res ,m.par.sd.true, n.est, store.env=T)
	
	# #calculate differences between invader and resident beta_0's - 
	# #the stoch_lambda_beta function works on differences
	
	# beta.diffs <- beta.s - beta.s[i]
	
	# #use mclapply to calculate the stochastic growth rates for all the different invaders
...
#the pips are a bit ragged as it's a stochastic simulation, lets smooth them a bit
#should use a 2D smoother but let's just do it on rows then columns.
...
text(-25,-15,expression(italic(lambda[S])*" > 1"),cex=1.1)
text(-11,-25,expression(italic(lambda[S])*" < 1"),cex=1.1)
text(-10,-15,expression(italic(lambda[S])*" > 1"),cex=1.1)
text(-11,-7,expression(italic(lambda[S])*" < 1"),cex=1.1)

#add some arrows showing the direction of evolution

points(-25,-25,pch=19)
arrows(-25,-25,-25,-20,length=0.1)
points(-20,-20,pch=19)
arrows(-25,-20,-20.05,-20,length=0.1)
arrows(-20,-20,-20,-14.34,length=0.1)
arrows(-20,-14.34,-14.34,-14.34,length=0.1)
points(-14.34,-14.34,pch=19)
add_panel_label("a")

mip <- smoothed.fit * t(smoothed.fit)

#set the diagonal ones to -1, the stochastic growth rate is zero here so all we see is 
#estimation error from the finite time series used to estimate lambda_S
...
#dev.copy2eps(file="../../c9/figures/CarlinaPip.eps")
